...
const CustomerSubscriptionReport = () => {
  ...
  const columnsConversion = [
    ...
    { field: 'TotalSignups', headerName: 'Total Signups', flex: 1 },
    { field: 'Purchases', headerName: 'Purchases After Signup', flex: 1 },
    { field: 'ConversionRate', headerName: 'Conversion Rate (%)', flex: 1 }
  ];
    ...
    <LocalizationProvider dateAdapter={AdapterDateFns}>
      <Box sx={{ p: 4, overflowY: 'auto' }}>
        <Paper sx={{ p: 4, backgroundColor: '#2b2b2b', color: '#fff' }}>
          ...
          <Tabs value={tab} onChange={(e, newVal) => setTab(newVal)} textColor="primary" indicatorColor="primary" sx={{ mb: 3 }}>
            ...
            <>
              ...
              <Box sx={{ height: 400, backgroundColor: '#1a1a1a', p: 2, mb: 4 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={reportResult}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#444" />
                    <XAxis dataKey="SignupDate" tickFormatter={formatDisplayDate} stroke="#ccc" />
                    <YAxis stroke="#ccc" label={{ value: 'Signups', angle: -90, position: 'insideLeft', offset: 10, fill: '#ccc' }} />
                    <Tooltip
                      contentStyle={{ backgroundColor: '#333', borderColor: '#555', color: '#fff' }}
                      labelFormatter={(label) => formatDisplayDate(label)}
                      formatter={(value, name) => [`${value}`, name === 'TotalSignups' ? 'Signups' : name]}
                    />
                    <Line type="monotone" dataKey="TotalSignups" stroke="#21AFBF" strokeWidth={2} dot={{ r: 4 }} />
                  </LineChart>
                </ResponsiveContainer>
              </Box>
          ...
          {tab === 2 && (
            <>
              ...
              <Box sx={{ height: 400, backgroundColor: '#1a1a1a', p: 2, mb: 4 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={reportResult} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#444" />
                    <XAxis dataKey="SignupDate" tickFormatter={formatDisplayDate} stroke="#ccc" />
                    <YAxis stroke="#ccc" />
                    <Tooltip formatter={(value) => value} labelFormatter={formatDisplayDate} />
                    <Bar dataKey="ConversionRate" fill="#21AFBF">
                      <LabelList dataKey="ConversionRate" position="top" fill="#fff" formatter={(v) => `${v}%`} />
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </Box>
